...
static char *hostname = NULL, *dbname = NULL, *dbuser = NULL, *password = NULL, *dbsock = NULL, *OptsAdm = NULL, *OptsUsr = NULL, *ConfApp = NULL, *ConfAppCount = NULL;
...
db_reconnect:
	...
	if (!connected) {
		...
		sprintf(sqlcmd,"SELECT b.roomNo,b.roomPass,b.maxUser,b.status,b.silPass,b.aFlags,b.uFlags,b.bookId,b.startTime FROM booking b WHERE b.roomNo = '%s' AND b.status = 'A' AND b.startTime <= '%s' AND (b.endTime >= '%s' OR b.endTime = '0000-00-00 00:00:00')", dtmfinput.inroom, currenttime, currenttime);
		ast_mutex_lock(&handle.lock);
		...
		{
			ast_log(LOG_ERROR,"CBMySQL: MySQL Error response: \n");
			return  0;
		}
...
static int load_config(void)
{
	...
	if (tmp) {
		...
		dbname = malloc(strlen("conferencedb") + 1);
		if (dbname != NULL) {
			ast_log(LOG_WARNING,"MySQL database not specified.  Assuming conferencedb\n");
			dbname_alloc = 1;
			strcpy(dbname,"conferencedb");
		} else {
			ast_log(LOG_ERROR,"Out of memory error.\n");
			return -1;
		}
